...
if (url.match(/action=set/)) {
  ...
  if (new_setting.type != "External")
    settings[service].external_subtitles = "null";
  if (new_setting.type == "Reset") new_setting = default_settings[service];
  if (new_setting.service && service == "General")
    settings[service].service = new_setting.service.replace(/\r/g, "");
  ...
  if (new_setting.dkey && service != "YouTube")
    settings[service].dkey = new_setting.dkey;
  if (new_setting.s_subtitles_url)
    settings[service].s_subtitles_url = new_setting.s_subtitles_url;
  if (new_setting.t_subtitles_url)
    settings[service].t_subtitles_url = new_setting.t_subtitles_url;
  if (new_setting.subtitles)
    settings[service].subtitles = new_setting.subtitles;
  if (new_setting.subtitles_type)
    settings[service].subtitles_type = new_setting.subtitles_type;
  if (new_setting.subtitles_sl)
    settings[service].subtitles_sl = new_setting.subtitles_sl;
  if (new_setting.subtitles_tl)
    settings[service].subtitles_tl = new_setting.subtitles_tl;
  if (new_setting.subtitles_line)
    settings[service].subtitles_line = new_setting.subtitles_line;
  if (new_setting.external_subtitles)
    settings[service].external_subtitles =
      new_setting.external_subtitles.replace(/\r/g, "");
...
if (
  service == "Netflix" &&
  !body.match(/\d+:\d\d:\d\d.\d\d\d -->.+line.+\n.+/g)
)
  $done({});
...
if (service == "YouTube") {
  ...
  if (
    url
      .replace(/&lang=zh(-Hans)*&/, "&lang=zh-CN&")
      .replace(/&lang=zh-Hant&/, "&lang=zh-TW&")
      .match(patt) ||
    url.match(/&tlang=/)
  )
    $done({});
    ...
    for (var i in timeline) {
      ...
      if (body.match(patt) && response.body.match(patt)) {
        ...
        if (setting.line == "f")
          body = body.replace(
            patt,
            `${timeline[i]}${response.body.match(patt)[1]}\n$1</p>`
          );
      }
    }
...
if (setting.type == "Official" && url.match(/\.m3u8/)) {
  ...
  let patt = new RegExp(
    `TYPE=SUBTITLES.+NAME="${setting.tl.replace(
      /(\[|\]|\(|\))/g,
      "\\$1"
    )}.+URI="([^"]+)`
  );
  ...
  if (body.match(patt)) {
    ...
    if (service == "Disney")
      host = url.match(
        /https.+media.(dss|star)ott.com\/ps01\/disney\/[^\/]+\//
      )[0];

    let subtitles_data_link = `${host}${body.match(patt)[1]}`;

    if (service == "PrimeVideo") {
      correct_host = subtitles_data_link.match(
        /https:\/\/(.+(cloudfront|akamaihd|avi-cdn).net)/
      )[1];
    ...
    $task.fetch(options).then((response) => {
      let subtitles_data = "";
      if (service == "Disney")
        subtitles_data = response.body.match(/.+-MAIN.+\.vtt/g);
      if (service == "HBOMax")
        subtitles_data = response.body.match(/http.+\.vtt/g);
      if (service == "PrimeVideo")
        subtitles_data = response.body.match(/.+\.vtt/g);

      if (service == "Disney") host = host + "r/";
      if (service == "PrimeVideo")
        host = subtitles_data_link.match(/https.+\//)[0];

      if (subtitles_data) {
        subtitles_data = subtitles_data.join("\n");
        if (service == "Disney" || service == "PrimeVideo")
          subtitles_data = subtitles_data.replace(/(.+)/g, `${host}$1`);
      ...
      if (
        service == "Disney" &&
        subtitles_data_link.match(/.+-MAIN.+/) &&
        response.body.match(/,\nseg.+\.vtt/g)
      ) {
        subtitles_data = response.body.match(/,\nseg.+\.vtt/g);
        let url_path = subtitles_data_link
          .match(/\/r\/(.+)/)[1]
          .replace(/\w+\.m3u8/, "");
        settings[service].t_subtitles_url = subtitles_data
          .join("\n")
  ...
  if (type == "DeepL") {
    for (var l in s_sentences) {
      let options = {
        ...
        method: "POST",
        body: `auth_key=${setting.dkey}${
          setting.sl == "auto" ? "" : `&source_lang=${setting.sl}`
        }&target_lang=${setting.tl}${s_sentences[l].join("")}`,
      };
...
function send_request(options) {
  return new Promise((resolve, reject) => {
    $task.fetch(options).then((response) => {
      resolve(
        options.method == "GET" ? response.body : JSON.parse(response.body)
      );
    });
  });
}
